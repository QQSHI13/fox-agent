...
- **A plugin is not a child process — it is code inside fox.** It is imported into fox-agent's own process with your full environment, API key included, and can do anything fox can. That is why `plugins` is read from `~/.config/fox-agent/config.toml` only and ignored (with a warning) in a project `fox-agent.toml`: cloning a repo and running fox in it must never execute that repo's code. Vet a plugin the way you would vet a shell profile.
